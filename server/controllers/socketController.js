...
module.exports.initializeUser = async (socket) => {
  ...
  const msgStr = await redisClient.lrange(`chat:${socket.user.userid}`, 0, -1);

  const messages = msgStr.map((m) => {
    const fields = m.split("|");
    return { to: fields[0], from: fields[1], content: fields[2] };
  });
...
module.exports.dm = async (socket, message) => {
  ...
  //to|from|content
  const msgStr = [message.to, message.from, message.content].join("|");
